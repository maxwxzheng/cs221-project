...
budgets for the movie.  We have budget data for 5,278 movies. We only consider
movies with budgest in US dollars, and we take the largest proposed budget.  We
also consider using budgets in two ways -- using raw dollar values or bucketing
the value into \$5M chunks (as indicator variables).
...
\end{itemize}
This results in 169,973 unique features across 9,883 movies, with an average of
72 features per movie. As a next step, we can analyze the importance of each
...
genre associated with a movie in the IMDb dataset. We have 25 genres across
9,885 movies, with an average of 2 genres per movie.
...
IMDb dataset. We have 11,701 unique keywords across 9,774 movies, with an
average of 75 features per movie.
...
features, one for the month, year, and both month and year. We have data for
9,884 movies, and this generates 10,5000 unique features across movies, an
average of 7 per movie.
...
\begin{table}[ht]
\caption{Results using only a single feature} % title of Table
...
Genre & 2.26 & 2.29 \\
Keywords & 1.65 & 3.35 \\
Release date & 1.69 & 2.26 \\ [1ex] % [1ex] adds vertical space
...
cast, keywords and release date. These are also our largest feature sets.

\begin{table}[ht]
\caption{Feature set sizes} % title of Table
\centering % used for centering table
\begin{tabular}{l r r r} % columns
\hline\hline
Feature             & Num. vars & Num. movies & Feature / movie  \\ [0.5ex] % inserts table 
\hline
Budget (raw)        & N/A & 5,278 (53\%) & 1 \\ % inserting body of the table
Budget (bucketized) & i & 5,278 (53\%) & 1 \\
Cast                & 169,973 & 9,883 (99\%) & 72 \\
Genre               & 25 & 9,885 (99\%) & 2 \\
Keywords            & 11,701 & 9,774 (98\%) & 75 \\
Release date        & 10,500 & 9,884 (99\%) & 7 \\ [1ex] % [1ex] adds vertical space
\hline %inserts single line
\end{tabular}
\end{table}
...
\begin{table}[ht]
\caption{Effect of pruning on linear regression} % title of Table
\centering % used for centering table
\begin{tabular}{l r r r} % columns
\hline\hline
...
As a result, removing the features that appear only once in training set gives huge improvement on performance and still has low standard error on fitting test set. This makes sense because if a feature appears only once in the training set, it's highly likely that the feature won’t appear in the test set, which means the weight for this feature won’t affect the result of fitting test set. Also according to the experiment, increasing the threshold n doesn’t give too much performance gain but worsens the test error.
\\
We plan to prune out features that are very specific to reduce the iteration time for our
algorithms and the likelihood of over-fitting.  Rather than removing entire
feature sets, we have started to consider using principal component analysis
(PCA) to algorithmically reduce our data sizes.
...
\begin{table}[ht]
\caption{Model performance} % title of Table
\centering % used for centering table
